...
import { User, Shield, PenTool, Mail, Calendar, CheckCircle, XCircle, Crown } from 'lucide-react';
...
const Members = () => {
  ...
  // Realtime: refrescar lista cuando cualquier perfil cambie
  useEffect(() => {
        ...
        {filtered.map((m, i) => {
          ...
          return (
            <div key={m.id} className="glass member-card animate" style={{
              ...
              {/* Admin controls — never shown for yourself */}
              {role === 'admin' && !isSelf && (
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  ...
                  >
                    {actionLoading === `status-${m.id}` ? '...'
                      : isInactive ? <><CheckCircle size={13} /> Activar</> : <><XCircle size={13} /> Desactivar</>}
                  </button>
                  <button
                  ...
                  >
                    {actionLoading === `role-${m.id}` ? '...'
                      : isAdmin ? <><Shield size={13} /> Quitar admin</> : <><Crown size={13} /> Hacer admin</>}
                  </button>
                </div>
              )}
            </div>
